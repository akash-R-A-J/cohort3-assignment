...
const Signin = () => {
  ...
  const handleSubmit = async (e?: React.FormEvent<HTMLButtonElement>) => {
    ...
    if (!email.current || !password.current) {
      ...
      // Do not show loading toast if fields are empty
      return; // Stop execution if fields are empty
    }

    // Show loading toast only if both fields are filled
    ...
    toast.dismiss(loadId); // Dismiss the loading toast

    if (!res?.error) {
      router.push('/'); // Redirect if sign-in is successful
